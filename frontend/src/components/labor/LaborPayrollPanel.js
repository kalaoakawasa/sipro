...
const RECORD_ROLES = ["project_manager", "site_engineer", "super_admin", "owner"];
const APPROVE_ROLES = ["finance", "finance_manager", "super_admin", "owner"];
const monthStart = () => {
  const d = new Date();
...
export default function LaborPayrollPanel({ projectId: fixedProject, mode = "field",
  ...
  const canRecord = RECORD_ROLES.includes(user?.role);
  ...
  return (
    <div data-testid={LABOR.payrollPanel} className="space-y-3">
      ...
      <PayrollDetailSheet payrollId={detailId} open={!!detailId}
        onOpenChange={(v) => !v && setDetailId(null)} canRecord={canRecord}
