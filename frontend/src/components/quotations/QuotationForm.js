...
export default function QuotationForm({ open, onOpenChange, leadId, source, onDone }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get("/units", { params: { status: "available", limit: 200 } }),
      api.get("/finance/config/payment-schemes"),
      api.get("/catalog/addons", { params: { active: true } }).catch(() => ({ data: { data: [] } })),
    ]).then(([u, s, a]) => {
      const list = u.data.data || [];
